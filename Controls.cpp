...
void Createbuttons(RECT *rc) {
  ...
  hclose=CreateWindow("BUTTON","Exit",
    WS_VISIBLE|WS_CHILD|BS_PUSHBUTTON,
...
HWND Createprogressbar(RECT *rc) {
  ...
  sprintf(message,"READY",VERSIONHI,VERSIONLO);
  showpercent=0;
  // Create bar.
...
void Displayblockimage(t_procdata *pdata,int posx,int posy,
  ...
  if (pdata!=NULL && pdata->unsharp!=NULL && pdata->xstep>0 && pdata->ystep>0) {
    ...
    if (answer>=0) {                   // Block recognized
      ...
      xstep=pdata->xstep;
      xpeak=(bufdx-xstep)/2.0f;
      ystep=pdata->ystep;
      ypeak=(bufdy-ystep)/2.0f; };
    buf=pdata->unsharp;
    ...
    scale=(int)min(blockdx/(pdata->xstep*1.1),blockdy/(pdata->ystep*1.1));
    if (scale<1) scale=1;
    fscale=(float)scale;
    // Calculate offset between display amd buffer (pdata->unsharp) coordinates.
    offsetx=xpeak+(xstep-blockdx/fscale)*0.5f;
    if (blockdy<blockdx)               // Draw in the middle
      offsety=ypeak+(ystep-blockdy/fscale)*0.5f;
    else                               // Align to top
      offsety=ypeak+(ystep-(2*blockdy-blockdx)/fscale)*0.5f;
    // Draw block.
    x0=(int)((xpeak-xstep*0.1-offsetx)*fscale+0.5f);
    if (x0<0) x0=0;
    x1=(int)((xpeak+xstep*1.1-offsetx)*fscale+0.5f);
    if (x1>blockdx) x1=blockdx;
    y0=(int)((ypeak-ystep*0.1-offsety)*fscale+0.5f);
    if (y0<0) y0=0;
    y1=(int)((ypeak+ystep*1.1-offsety)*fscale+0.5f);
    if (y1>blockdy) y1=blockdy;
    for (y=y0; y<y1; y++) {
      bufy=(int)(y/fscale+offsety);
      if (bufy<0) continue;
      if (bufy>=bufdy) break;
      pbuf=buf+bufy*bufdx;
      pblock=blockbits+y*blockdx+x0;
      for (x=x0; x<x1; x++,pblock++) {
        bufx=(int)(x/fscale+offsetx);
        if (bufx<0) continue;
        if (bufx>=bufdx) break;
    ...
    if (answer>=0 && blockdotmode!=BSEL_NONE) {
      x0= (int)((xpeak-offsetx)*fscale+1.5f);
      x1= (int)((xpeak+xstep-offsetx)*fscale+1.5f);
      y0= (int)((ypeak-offsety)*fscale+1.5f);
      y1= (int)((ypeak+ystep-offsety)*fscale+1.5f);
      for (x=x0; x<=x1; x++) {
        // Horizontal borders.
        if (x<0) continue;
        if (x>=blockdx) break;
    ...
    if (blockdotmode!=BSEL_NONE &&
      ...
      // vertical lines.
      y0= (int)((ypeak+ystep/(NDOT+3.0)*1.5-offsety)*fscale+1.5);
      if (y0<0) y0=0;
      y1= (int)((ypeak+ystep/(NDOT+3.0)*(NDOT+1.5)-offsety)*fscale+1.5);
      if (y1>=blockdy) y1=blockdy-1;
      for (i=0; i<=NDOT; i++) {
        x= (int)((xpeak+xstep/(NDOT+3.0)*(i+1.5)-offsetx)*fscale+1.5);
        if (x<0 || x>=blockdx) continue;
        pblock=blockbits+y0*blockdx+x;
        for (y=y0; y<=y1; y++,pblock+=blockdx) *pblock|=0x80; };
      // Draw horizontal lines.
      x0= (int)((xpeak+xstep/(NDOT+3.0)*1.5-offsetx)*fscale+1.5);
      if (x0<0) x0=0;
      x1= (int)((xpeak+xstep/(NDOT+3.0)*(NDOT+1.5)-offsetx)*fscale+1.5);
      if (x1>=blockdx) x1=blockdx-1;
      for (j=0; j<=NDOT; j++) {
        y= (int)((ypeak+ystep/(NDOT+3.0)*(j+1.5)-offsety)*fscale+1.5);
      ...
      for (j=0; j<NDOT; j++) {
        for (i=0; i<NDOT; i++) {
          ...
          if (u==0)
            continue;
          x0= (int)((xpeak+xstep/(NDOT+3.0)*(i+1.5)-offsetx)*fscale+1.5);
          x1= (int)((xpeak+xstep/(NDOT+3.0)*(i+2.5)-offsetx)*fscale+1.5);
          y0= (int)((ypeak+ystep/(NDOT+3.0)*(j+1.5)-offsety)*fscale+1.5);
          y1= (int)((ypeak+ystep/(NDOT+3.0)*(j+2.5)-offsety)*fscale+1.5);
